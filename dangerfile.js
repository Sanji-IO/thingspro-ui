...
const allChanges = [].concat(danger.git.created_files, danger.git.modified_files, danger.git.deleted_files);
...
if (i18nChanged && !potChanged) {
  ...
  const idea = 'Perhaps you need to run `$ yarn run extract:messages && yarn run extract:pot`?';
